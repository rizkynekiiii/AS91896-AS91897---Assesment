...
def print_hired_details():
    ...
    Label(main_window, text="Receipt Number").grid(column=6, row=10)
    Label(main_window, text="Number of Hired").grid(column=4, row=10)
    Label(main_window, text="Price").grid(column=5, row=10)
...
def setup_buttons():
    global hired_details, entry_full_name, entry_item, entry_number_hired, entry_price, entry_receipt_num, entry_row, entry_price_num, entry_number, entry_cost
    Button(main_window, text="Print Details",command=print_hired_details, bg="cyan").grid(column=3, row=2)
    Button(main_window, text="Quit",command=quit, bg="hot pink").grid(column=7, row=2)
    Button(main_window, text="Append",command=append_details, bg="cyan").grid(column=2,row=2)
    Button(main_window, text="Calculate Cost",command=calculate_cost, bg="cyan").grid(column=5, row=2)
    Button(main_window, text="Delete Row",command=delete_row, bg="hot pink").grid(column=4,row=9)
    Label(main_window, font='bold',text="~ Julie's Party Hire ~").grid(column=4, row=0)
    ...
    Label(main_window, text="Cost :").grid(column=2, row=8)
    entry_cost = Entry(main_window)
    entry_cost.grid(column=3, row=8)
    Label(main_window, text="Row #").grid(column=2, row=9)
    entry_row = Entry(main_window)
    entry_row.grid(column=3, row=9)
    Label(main_window, text="Price").grid(column=5,row=3)
    entry_price_num = Entry(main_window)
    entry_price_num.grid(column=5, row=4)
    Label(main_window, text="Num/Hired").grid(column=6, row=3)
    entry_number = Entry(main_window)
    entry_number.grid(column=6, row=4)
    Label(main_window, text="Item/Price").grid(column=5,row=6)
    Label(main_window, text="Receipt Input").grid(column=6,row=6)
...
def calculate_cost():
    global j_names, total_entries, name_count, entry_price2, entry_number #Labels here will be printer inot the main_window
    cost = int(entry_price_num.get())* int(entry_number.get())
    Label(main_window, text=cost).grid(column=7, row=4)
    

#def append_details():
def append_details():
    global hired_details, entry_full_name, entry_item, entry_number_hired, entry_price, entry_receipt_num, entry_row, total_entries, entry_price_num, entry_number, entry_cost
    if len(entry_full_name.get()) != 0 :
        hired_details.append([entry_full_name.get(),entry_item.get(),entry_number_hired.get(),entry_price.get(),entry_receipt_num.get(),entry_cost.get(),entry_row.get()])
...
#Items and their price combobox
price = StringVar()
ComboBox = ttk.Combobox(main_window, textvariable=price,
                        values=("Cutlery Set 1x = $2 ", "Table/Cloth 1x = $5", "Chair 1x = $3", "Plate/Glassware 1x = $2", "Napkins 1x = $1", "Lighting 1x = $6", "12m Carpet 1x = $10", "Ballon Set 1x = $4", "Catering Equip 1x = $3", "Cupcake Holder 1x = $4"), state='readonly').grid(column=5, row=7)
#Receipt Num Input
receipt_num = StringVar()
ComboBox = ttk.Combobox(main_window, textvariable=receipt_num,
                        values=("01234", "12345", "23456", "34567", "45678", "56789", "67890", "78901", "89012", "90123"), state='readonly').grid(column=6, row=7)
